...
int main()
{
	...
	int points_per_row = 10;                           // 标定版每行的内点数
	int points_per_col = 7;                            // 标定版每列的内点数
	Size image_size;                                   // 图片尺寸
	Size corner_size(points_per_row, points_per_col);  // 标定板每行每列角点个数，共11*8个角点
	...
	{
		image_raw = imread(entry.path().string());                                             // 按照RGB图像读取数据
	...
	Size block_size(16, 16);                         // 每个小方格实际大小16mm,(w,h)
